...
/**
 * Engraving editor trigger and modal.
 *
 * @description Displays the current engraving (if set) with an option to edit
 * it, or an "Add Engraving" button if none exists. Opens a modal containing the
 * EngravingPanel with text input, font selection, and a live SVG preview.
 * Manages modal mount/unmount with AnimatePresence.
 *
 * @param engraving - Current engraving data (text + fontStyle).
 * @param setEngraving - Setter for engraving state.
 * @returns The engraving trigger and modal.
 */
            ...
            >
              <div className="space-y-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-semibold text-gray-900">Add Engraving</h3>
                  <motion.button
                    onClick={() => setShowEngravingModal(false)}
...
const EngravingPanel: React.FC<{
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-[#502B3A] mb-2">Choose Font</label>
        <div className="grid grid-cols-6 gap-2">
          {fonts.map((font) => (
            <motion.button
              key={font.name}
              className={`py-2 px-3 border text-lg border-gray-300 ${
                engraving?.fontStyle === font.fontFamily || selectedFont.name === font.name
      ...
      <EngravingPreview text={engravingText} fontFamily={selectedFont.fontFamily} />
...
const EngravingPreview: React.FC<EngravingPreviewProps> = ({ text, fontFamily }) => {
  return (
    <div className="relative w-full mb-3">
      ...
      >
        ...
        >
          <path id="engravingPath" d="M0,80 Q124,34 248,80" fill="transparent" stroke="none"></path>
          <text
            textAnchor="middle"
 ...
 *
 * @description A modal dialog for confirming destructive delete actions.
 * Features an animated overlay, a title with warning icon, a customizable
 * message (or auto-generated one from itemName), cancel and delete buttons,
 * and a loading state on the delete button. Closes on Escape key.
 *
 * @param isOpen - Whether the modal is visible.
 * @param onClose - Callback to close the modal.
 * @param onConfirm - Callback executed on delete confirmation.
 * @param title - Modal heading (default "Delete Item").
 * @param message - Custom confirmation message.
 * @param isDeleting - Shows "Deleting..." on the delete button when true.
 * @param itemName - Item name for auto-generating a message.
 * @returns A confirmation modal overlay.
 */
          ...
          >
            ...
            >
              ...
              <div className="flex-1 px-6 py-5 min-h-[80px] flex items-start">
                <p className="text-sm text-gray-600 leading-relaxed">{message || defaultMessage}</p>
              </div>
