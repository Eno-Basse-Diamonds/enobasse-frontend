 ...
 * Ring size selector.
 *
 * @description A dropdown ring size picker with an optional link or button to
 * open a size guide modal. The modal includes a full-view toggle. Closes on
 * click outside and manages header z-index to prevent overlap.
 *
 * @param selectedSize - Currently selected ring size.
 * @param onSetSelectedSize - Callback when a size is chosen.
 * @param isDropdown - When false, uses a link instead of a modal button.
 * @param sizeGuideHref - URL for the link-based size guide.
 * @returns A ring size dropdown with optional modal guide.
 */
export const RingSizeSelector: React.FC<RingSizeSelectorProps> = ({
  ...
  return (
    <div className="flex flex-row items-center gap-x-3 z-50">
      ...
      {isDropdown ? (
        <button onClick={openModal} className="font-medium text-[#502B3A] text-sm">
          Size Guide
        </button>
      ) : (
        <Link href={sizeGuideHref} className="font-medium text-[#502B3A] text-sm hover:underline">
          Size Guide
        </Link>
      )}
