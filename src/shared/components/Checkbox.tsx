...
import { Gemstone, Metal } from "@/modules/products/types";
 ...
 * Metal type selector.
 *
 * @description Renders a row of circular buttons for selecting a precious metal
 * (White Gold, Yellow Gold, Rose Gold, Platinum). Each option displays its purity
 * abbreviation and is color-coded. Includes hover preview and spring animations
 * on the active selection ring.
 *
 * @param metalOptions - Available metal choices with type and purity.
 * @param selectedMetal - Currently selected metal.
 * @param onSelectMetal - Callback fired when a metal is clicked.
 * @returns Interactive metal type picker.
 */
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-full p-[2px] ${
                  isActive ? `${colors.border} border-2` : "border-transparent border-2"
                }`}
              ...
              >
                <motion.button
                  className={`size-8 flex items-center justify-center rounded-full text-xs text-[#502B3A] ${
                    isActive ? `${colors.bg}` : `bg-white border-2 ${colors.border}`
                  }`}
                    ...
                    style={{
                      borderColor: colors?.border
                        ? colors.border.includes("border-[")
                          ? colors.border.split("border-[")[1]?.replace("]", "") || "#000"
                          : colors.border
                        : "#000",
                    }}
 ...
 *
 * @description Renders a row of circular buttons for selecting a gemstone
 * (Diamond, Sapphire, Ruby, Emerald). Each option shows its carat weight and is
 * color-coded. Manages internal state when no external selection is provided.
 * Includes hover preview and spring animations.
 *
 * @param gemstoneOptions - Available gemstone choices with type and weight.
 * @param selectedGemstone - Externally controlled selected gemstone.
 * @param onSelectGemstone - Callback fired when a gemstone is clicked.
 * @returns Interactive gemstone picker.
 */
export const GemstoneSelector: React.FC<GemstoneOptionsProps> = ({
  ...
  const [internalSelectedGemstone, setInternalSelectedGemstone] = useState(gemstoneOptions[0]);
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    className="absolute inset-0 border-[0.5px] rounded-full"
                    style={{
                      borderColor: colors.border.split("border-[")[1].replace("]", ""),
                    }}
