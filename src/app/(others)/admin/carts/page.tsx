...
import { useDashboardCarts } from "@/modules/admin/hooks";
...
export default function AdminCartsPage() {
  ...
  const { data: carts, isLoading, error } = useDashboardCarts();
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mb-8">
          <h3 className="text-lg font-medium text-gray-900">
            Customer Carts ({carts?.length || 0})
          </h3>
          <p className="text-sm text-gray-500">
            Cart items belonging to signed-in users
          </p>
        </div>

        {error ? (
          <div className="bg-red-50 border border-red-200 p-4">
            <p className="text-red-800">Error loading carts: {error.message}</p>
          </div>
        ) : isLoading ? (
          ...
          <div className="space-y-6">
            {carts.map((cart) => (
              <div key={cart.id} className="bg-white border border-primary-500/10 rounded-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-6 py-4 border-b border-gray-100">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{cart.account.email}</div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4 text-sm text-gray-500">
                    <span className="inline-flex items-center px-2 py-1 bg-orange-50 text-orange-700 font-medium">
                      <ShoppingBag className="w-3.5 h-3.5 mr-1" />
                      {cart.itemCount} {cart.itemCount === 1 ? "item" : "items"}
                    </span>
                    <span>Last updated {formatDate(cart.updatedAt)}</span>
                  </div>
                </div>

                <div className="divide-y divide-gray-100">
                  {cart.items.length === 0 ? (
                    <div className="px-6 py-6 text-sm text-gray-500">No items in this cart.</div>
                  ) : (
                    cart.items.map((item) => (
                          ...
                          <div className="flex flex-wrap items-start justify-between gap-2">
                            <div className="min-w-0">
                              ...
                              <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-400">
                                ...
                                {item.engraving?.text && (
                                  <span>Engraving: &ldquo;{item.engraving.text}&rdquo;</span>
                                )}
                              </div>
                            </div>
                            <div className="text-sm font-medium text-gray-900">
          ...
          <EmptyState
            ...
            description="Carts belonging to signed-in users will appear here."
