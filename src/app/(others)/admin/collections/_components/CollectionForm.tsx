...
import { useCreateCollection, useUpdateCollection } from "@/modules/collections/hooks";
import { CollectionFormData, CollectionFormSchema } from "@/modules/collections/schemas";
...
export function CollectionForm({ collection, onClose }: CollectionFormProps) {
  ...
  const handleImageChange = (field: keyof typeof formData.image, value: string) => {
    setFormData((prev) => ({
      ...prev,
    ...
    <>
      ...
      >
        <form id="collection-form" onSubmit={handleSubmit} className="space-y-5">
          ...
          <ImageUploadField formData={formData} errors={errors} onImageChange={handleImageChange} />
...
const ImageUploadField = ({
  ...
  <div>
    <label className="block text-sm font-semibold text-primary-400 mb-1.5">Featured Image *</label>
