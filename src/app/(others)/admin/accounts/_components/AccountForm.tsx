...
import { Account, CreateAccountData, UpdateAccount } from "@/modules/admin/types";
...
export function AccountForm({ account, onClose }: AccountFormProps) {
  ...
  const isFormValid =
    !isPending && formData.name.trim() && formData.email.trim() && (account || formData.password);
    ...
    <>
      ...
      >
        <form id="account-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          <section>
            <h4 className="text-base font-semibold text-primary-500 mb-4">Account Settings</h4>
