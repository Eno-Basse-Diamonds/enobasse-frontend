        ...
        {success ? (
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600 max-w-md mx-auto leading-relaxed">
                Thank you! Your custom ring configuration has been successfully submitted. An Eno Bassé private jeweler will review your design and follow up with a bespoke quote shortly.
              </p>
            </div>

            <div className="w-full max-w-md bg-gray-50 rounded-sm p-4 border border-gray-100 text-left space-y-2.5">
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold">Submitted Design Summary</p>
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
                <div><span className="text-gray-400 font-medium">Gemstone:</span> <span className="text-gray-700 capitalize font-medium">{configuration.gemstoneShape} ({configuration.previewSize})</span></div>
                <div><span className="text-gray-400 font-medium">Metal:</span> <span className="text-gray-700 capitalize font-medium">{configuration.karat} {configuration.metalType.replace(/-/g, " ")}</span></div>
                <div><span className="text-gray-400 font-medium">Head Type:</span> <span className="text-gray-700 capitalize font-medium">{configuration.headStyle.replace(/-/g, " ")}</span></div>
                <div><span className="text-gray-400 font-medium">Shank Style:</span> <span className="text-gray-700 capitalize font-medium">{configuration.shankStyle.replace(/-/g, " ")}</span></div>
                {configuration.engravingText && (
                  <div className="col-span-2 border-t border-gray-100 pt-2 mt-1">
                    ...
                    <span className="text-secondary-500 font-medium block italic" style={{ fontFamily: configuration.engravingFont || 'inherit' }}>
                      "{configuration.engravingText}"
                    </span>
                  </div>
                )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <div>
                ...
                {generatedImages[0]?.src ? (
                  <div className="relative aspect-square w-full rounded-sm overflow-hidden bg-white border border-gray-100 flex items-center justify-center p-2 mb-4 shadow-sm group">
                    ...
                    />
                    <span className="absolute bottom-1.5 right-2 text-[9px] text-gray-300 font-mono tracking-wider uppercase bg-white/75 px-1 rounded-sm">3D Render</span>
                  </div>
                ) : (
                ...
                <div className="space-y-2.5 text-xs text-gray-600">
                  <div className="flex justify-between py-1 border-b border-gray-100/50">
                    ...
                    <span className="text-primary-500 font-semibold capitalize">{configuration.gemstoneShape} ({configuration.previewSize})</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-100/50">
                    <span className="font-medium text-gray-400">Diamond Type</span>
                    <span className="text-primary-500 font-semibold capitalize">{configuration.diamondType === 'lab' ? 'Lab-grown' : configuration.diamondType}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-100/50">
                    <span className="font-medium text-gray-400">Setting Style</span>
                    <span className="text-primary-500 font-semibold capitalize truncate max-w-[120px]" title={configuration.headStyle}>{configuration.headStyle.replace(/-/g, " ")}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-100/50">
                    <span className="font-medium text-gray-400">Shank Style</span>
                    <span className="text-primary-500 font-semibold capitalize truncate max-w-[120px]" title={configuration.shankStyle}>{configuration.shankStyle.replace(/-/g, " ")}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-100/50">
                  ...
                  <div className="flex justify-between py-1 border-b border-gray-100/50">
                    ...
                    <span className="text-primary-500 font-semibold">Size {configuration.ringSize}</span>
                  </div>
                  {configuration.engravingText && (
                    <div className="mt-2.5 p-2 bg-white rounded-sm border border-gray-100">
                      <span className="block text-[9px] text-gray-400 uppercase tracking-widest font-semibold mb-0.5">Engraving</span>
                      <span className="text-xs font-semibold text-secondary-500 block truncate" style={{ fontFamily: configuration.engravingFont || 'inherit' }}>
                        "{configuration.engravingText}"
                      </span>
                    </div>
                  )}
            ...
            <div className="md:col-span-3 space-y-4">
              <div>
                <h3 className="text-lg font-serif font-semibold text-primary-500 tracking-wide">Design Consultation</h3>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
                  Enter your details below. A bridal concierge will review your selections and prepare a bespoke quote.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3.5">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">
                    Phone Number <span className="text-[10px] text-gray-400 lowercase normal-case">(optional)</span>
                  </label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">
                    Special Requests <span className="text-[10px] text-gray-400 lowercase normal-case">(optional)</span>
                  </label>
                  <div className="relative">
