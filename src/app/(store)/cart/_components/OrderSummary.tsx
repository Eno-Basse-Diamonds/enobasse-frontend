...
export const OrderSummary: React.FC<OrderSummaryProps> = ({ items, onCheckout }) => {
  ...
  const displayCurrency =
    items.length > 0 ? items[0].productVariant.currency || preferredCurrency : preferredCurrency;

  return (
    <div className="bg-gray-50 p-6 sm:p-8 mx-auto flex-1 space-y-6 lg:mt-0 lg:w-full mt-6 sm:mt-8 rounded-sm">
      <h2 className="text-xl font-semibold text-[#502B3A] mb-4">Order Summary</h2>
      ...
      <div className="mt-4 text-center text-[#502B3A]/70">
        or{" "}
        <Link href="/products" className="hover:text-[#502B3A] text-sm font-medium">
          Continue Shopping
        </Link>
      </div>
    </div>
