...
import { QuantityControl } from "@/app/(store)/cart/_components/QuantityControl";
import { SizeSelect } from "@/app/(store)/cart/_components/SizeSelect";
...
export function CheckoutCartItem({ item, ringSizes, currency }: CheckoutCartItemProps) {
  ...
  const handleDecrement = () => {
    if (item.quantity > 1) {
      updateItem(item.productVariant.id, { quantity: item.quantity - 1 }, accountEmail);
    }
  };
  ...
  const metalStr =
    metals && metals.length > 0 ? `${metals[0].purity || ""} ${metals[0].type || ""}` : "";
  const gemstoneStr =
  ...
  return (
    <li className="py-4 flex items-start gap-4">
      <div className="flex-shrink-0 h-20 w-20 sm:h-24 sm:w-24 overflow-hidden border border-gray-200 relative rounded-sm">
        <Link href={`/products/${item.productSlug}`} className="relative block w-full h-full">
          <Image
            src={imageUrl}
            alt={imageAlt}
            fill
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs sm:text-sm text-[#502B3A]/80 mt-1 truncate">
              {gemstoneStr && metalStr ? `${gemstoneStr} | ${metalStr}` : metalStr || gemstoneStr}
            </p>
            ...
            <div className="my-2 sm:my-3 flex flex-wrap items-center gap-3 sm:gap-4 justify-between">
              ...
              {isRing && (
                <SizeSelect
                  ...
                  onChange={(size) => updateItem(item.productVariant.id, { size }, accountEmail)}
                />
              )}
            </div>
            ...
            {canBeEngraved && (
              <div className="mb-2">
                <Engraving
                  ...
                  setEngraving={(val) =>
                    updateItem(item.productVariant.id, { engraving: val }, accountEmail)
                  }
                />
              </div>
            )}
